import {Aspect, ControlCenter, AspectConfiguration, AspectSelection, VersionedObjectSnapshot} from '@openmicrostep/aspects';
import './resource';
...
function property_access_warm_opt(v) {
  ...
  v.manager().setSavedIdVersion(1, 1);
  i = ONE_K;
  while (i-- > 0) {
    v.name();
    v._firstname;
...
function property_access_10_000_000_000_mixed() { // around 100ms
  ...
  v1.manager().setSavedIdVersion(1, 1);
  let v2 = People.Aspects.test1.create(ccc);
